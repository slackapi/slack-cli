        ...
        if [ -x "$(command -v $SLACK_CLI_NAME)" ] ; then
                ...
                if [[ ! $($SLACK_CLI_NAME _fingerprint) == $FINGERPRINT ]] &>/dev/null ; then
                        ...
                        if [ "$SLACK_CLI_DEV_VERSION" == "dev" ]; then
                                delay 0.3 "🍀 It is the same Slack CLI! Upgrading to the latest development build..."
                        else
                                delay 0.3 "🍀 It is the same Slack CLI! Switching it with v$SLACK_CLI_DEV_VERSION..."
                        fi
                fi
        fi
