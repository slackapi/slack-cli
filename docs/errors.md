...
If you are using a Bolt framework, add a deploy hook then run: `slack-cli deploy`

Otherwise start your app for local development with: `slack-cli run`
...
**Remediation**: You need to be logged in to at least 1 production (slack.com) team to use this command. Log into one with the `slack-cli login` command and try again.
...
**Remediation**: You can upgrade to the latest version of the Slack CLI using the command: `slack-cli upgrade`
...
**Remediation**: Use the command `slack-cli login` to login to this workspace
...
**Remediation**: Use --search flag: `slack-cli docs --search "<query>"`
...
**Remediation**: View the feedback options with `slack-cli feedback --help`
...
View feedback options with `slack-cli feedback --help`
...
**Remediation**: Install the app with `slack-cli install`
...
If this is a Slack project, you can initialize it with `slack-cli init`
...
**Remediation**: Your user account authorization may be expired or does not have permission to access the resource. Try to login to the same user account again using `slack-cli login`.
...
**Remediation**: The previous slash command and challenge code have now expired. To retry, use `slack-cli login`, paste the slash command in any Slack channel, and enter the challenge code displayed by Slack. It is easiest to copy & paste the challenge code.
...
Use `slack-cli datastore --help` for examples
...
Read about manifest sourcing with the `slack-cli manifest info --help` command
...
**Remediation**: List your sandboxes with the `slack-cli sandbox list` command to find the ID
...
**Remediation**: Run `slack-cli logout` and then `slack-cli login` again.
...
**Remediation**: Authorize your CLI with `slack-cli login`
...
**Remediation**: Run `slack-cli doctor` to check that your system dependencies are up-to-date.
...
You can create a hooks file manually or with the `slack-cli init` command.
...
**Remediation**: Use the command `slack-cli login` to login and `slack-cli install` to install your app
...
**Remediation**: Use the command `slack-cli login` to authenticate again
